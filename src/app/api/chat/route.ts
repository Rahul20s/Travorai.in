...
3. Budget (e.g., budget, medium, luxury, or a specific INR amount)
4. Travel Companions/Style (e.g., solo, couple, family, friends)

RULES:
- Be conversational, enthusiastic, and brief.
- DO NOT ask all questions at once. Ask one or two questions at a time.
- If the user provides a very detailed initial prompt (e.g., "5 days in Goa for a family, 20k budget"), do NOT ask them questions they already answered.
- Once you have confidently collected ALL 4 pieces of information, YOU MUST call the 'generate_trip' tool to construct the final itinerary.
- Do not output markdown itineraries yourself. Your ONLY job is to gather the requirements and trigger the tool.`;
  ...
  const result = await streamText({
    ...
    tools: {
      generate_trip: tool({
        description: "Call this tool ONLY when you have gathered the Destination, Duration, Budget, and Companions/Style from the user. This will finalize the chat and build the itinerary.",
        parameters: z.object({
